<#
.SYNOPSIS
    Build script for MosyleFreeKit: analyze, test, and package.
.DESCRIPTION
    -Analyze   Run PSScriptAnalyzer against the source.
    -Test      Run the Pester suite.
    -Package   Flatten Public/Private into a single .psm1 under dist/MosyleFreeKit.
...
$sourceRoot = Join-Path $repoRoot 'src' 'MosyleFreeKit'
...
if ($Package -or $runAll) {
    ...
    $distRoot = Join-Path $repoRoot 'dist' 'MosyleFreeKit'
    if (Test-Path $distRoot) { Remove-Item $distRoot -Recurse -Force }
    New-Item -ItemType Directory -Path $distRoot -Force | Out-Null

    $builder = [System.Text.StringBuilder]::new()
    [void]$builder.AppendLine('Set-StrictMode -Version 3.0')
    [void]$builder.AppendLine('$script:DefaultMosyleFreeSession = $null')
    ...
    $manifest = Import-PowerShellDataFile (Join-Path $sourceRoot 'MosyleFreeKit.psd1')
    $exports = ($manifest.FunctionsToExport | ForEach-Object { "'$_'" }) -join ', '
    [void]$builder.AppendLine("Export-ModuleMember -Function @($exports)")

    Set-Content -Path (Join-Path $distRoot 'MosyleFreeKit.psm1') -Value $builder.ToString() -Encoding utf8BOM
    Copy-Item (Join-Path $sourceRoot 'MosyleFreeKit.psd1') $distRoot

    $job = Start-Job -ScriptBlock {
        param($path)
        Import-Module $path -Force
        (Get-Module MosyleFreeKit).ExportedFunctions.Keys
    } -ArgumentList (Join-Path $distRoot 'MosyleFreeKit.psd1')
